...
## Search for specific files on the network
...
Get-Childitem -Path $path_to_find -Include *.aep,*.ai,*.c4d,*.ma,*.mb,*.nk,*.nk_,*.hip,*.max,*.vrmesh -Recurse -ErrorAction SilentlyContinue | % {
    Clear-Host
    Write-Host "Searching... "
    $bytecount += $_.Length
...
$bytecount | Out-File -FilePath "C:\pwsh-scripts\serach.log"
```